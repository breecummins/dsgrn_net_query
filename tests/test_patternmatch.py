...
def test_patternmatch_ln_params():
    Path("temp_results").mkdir(exist_ok=True)
    command = " ".join(["mpiexec", "-n", "2", "python", "../src/dsgrn_net_query/queries/CountPatternMatch_large_networks_parameter_indices.py", "mpi_network_pm_ln.txt", "mpi_params_pm.json", "temp_results",">dsgrn_net_query.log","2>&1"])
    os.system(command)
    # qdir = subprocess.check_output("tail -n 1 dsgrn_net_query.log",shell=True).strip().decode("utf-8")
    qdir = open(".query_results.log").read().strip()
    output_file1 = os.path.join(qdir,"query_results_domain_wt1_microarray_coregenes_lifepoints_interpol_trim.json")
    ...
    output_file1 = os.path.join(qdir,"query_results_domain_wt1_microarray_coregenes_lifepoints_interpol_trim_param_list.json")
    output_file2 = os.path.join(qdir,"query_results_stablefc_wt1_microarray_coregenes_lifepoints_interpol_trim_param_list.json")
    output_file3 = os.path.join(qdir,"query_results_domain_wt_rnaseq_ts_param_list.json")
    output_file4 = os.path.join(qdir,"query_results_stablefc_wt_rnaseq_ts_param_list.json")
    output_file5 = os.path.join(qdir,"query_results_domain_all_param_list.json")
    output_file6 = os.path.join(qdir,"query_results_stablefc_all_param_list.json")
    results1 = json.load(open(output_file1))
    results2 = json.load(open(output_file2))
    results3 = json.load(open(output_file3))
    results4 = json.load(open(output_file4))
    results5 = json.load(open(output_file5))
    results6 = json.load(open(output_file6))
    assert(results1 == {"SWI4 : (NDD1)(~YOX1) : E\nHCM1 : SWI4 : E\nNDD1 : HCM1 : E\nYOX1 : SWI4 : E": [[0.0, []], [0.01, [0, 2, 5, 6, 7, 9, 12, 13]], [0.05, [0, 2, 5, 6, 13]]]})
    assert(results2 == {"SWI4 : (NDD1)(~YOX1) : E\nHCM1 : SWI4 : E\nNDD1 : HCM1 : E\nYOX1 : SWI4 : E": [[0.0, []], [0.01, [2, 12]], [0.05, [2]]]})
    assert(results3 == {"SWI4 : (NDD1)(~YOX1) : E\nHCM1 : SWI4 : E\nNDD1 : HCM1 : E\nYOX1 : SWI4 : E": [[0.0, []], [0.01, []], [0.05, []]]})
    assert(results4 == {"SWI4 : (NDD1)(~YOX1) : E\nHCM1 : SWI4 : E\nNDD1 : HCM1 : E\nYOX1 : SWI4 : E": [[0.0, []], [0.01, []], [0.05, []]]})
    assert(results5==results1)
    assert(results6==results2)
    subprocess.call(["rm","-r", "temp_results/"])
